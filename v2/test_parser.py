...
def main():
    ...
    print(f"● 공백 압축 후 본문 크기: {len(doc['text'])}자")
    print(f"\n[본문 처음 300자 미리보기]:\n{doc['text'][:300]}...")
    ...
    print("\n" + "="*50)
    print(f"=== 2. v2 청킹 분할 결과 (총 {len(chunks)}개 조각) ===")
    print("="*50)
    
    # 앞선 3개 청크만 예시로 출력
    for i, chunk in enumerate(chunks[:3]):
        print(f"\n[청크 조각 #{i+1}] (글자수: {len(chunk['text'])}자 | 인덱스: {chunk['metadata']['chunk_index']})")
        print("-" * 50)
        print(chunk['text'])
        print("-" * 50)
