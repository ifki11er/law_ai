...
from config.settings import AI_HUB_DATA_PATH, OLLAMA_LLM_MODEL, OLLAMA_BASE_URL, STANDARD_REFUSAL
...
def load_test_scenarios(labeled_path: str) -> list:
    """
    라벨링 데이터 폴더 내의 모든 _QA 폴더(결정례, 법령, 판결문, 해석례 등)를 탐색하여
    각 폴더별로 1개씩 골고루 테스트용 QA JSON 파일들을 샘플링하여 파싱합니다.
    """
    scenarios = []
    ...
    # "_QA"가 폴더명에 들어가는 경우만 선별 (결정례, 법령, 판결문, 해석례 등)
    ...
    for qdir in qa_dirs:
        ...
        # 각 카테고리 폴더에서 가장 첫 번째 JSON 파일 1개만 파싱하여 추가
        if json_files:
...
def run_evaluation():
    ...
    # 2. RAG 컴포넌트 로드
    print("[2단계] 로컬 RAG 시스템 엔진 및 채점관(Evaluator) 로드 중...")
    ...
    for idx, sc in enumerate(scenarios):
        ...
        # 1차 방어막 통과 여부 확인
        print(f"[Q{q_num} - 1차 방어막] Ollama/gemma4를 사용해 질문의 법률적 의도 분석 중...")
        is_legal = guardrail_agent.check_legal_intent(sc['question'])
        if not is_legal:
            print(f"[Q{q_num} - 1차 결과] ❌ 일반 질문으로 차단되어 평가 중단!")
            continue
        print(f"[Q{q_num} - 1차 결과] ✅ 법률 질문 판정 통과. 2단계로 진입.")
            
        # Chroma 검색 (Q&A에 나온 질문 원본 그대로를 사용하여 현실적인 검색 성능을 측정하되, k값을 10으로 늘려 결론이 실린 청크가 누락되지 않도록 합니다.)
        print(f"\n[Q{q_num} - 2차 검색] bge-m3 임베딩을 이용해 Chroma DB 유사도 검색 중...")
        raw_results = vector_db.search_with_scores(sc['question'], k=10)
        print(f"[Q{q_num} - 2차 결과] 검색 후보군 {len(raw_results)}개 획득.")
        
        # 3차 가드레일 (유사도 컷오프)
        print(f"\n[Q{q_num} - 3차 방어막] 임계값(1.1) 미만 유사도를 가진 관련성 높은 조항 선별 중...")
        ...
        # 2차 가드레일 및 답변 생성
        print(f"\n[Q{q_num} - 4차 답변 생성] 필터링된 근거들을 조립하여 Ollama/gemma4로 법률 답변 생성 중...")
    ...
    # 4. 가짜(무관한) 질문 방어력 추가 테스트
    print("\n" + "="*60)
    print("=== [4단계] 가드레일 우회 및 오작동 방어력 테스트 ===")
    print("="*60)
    ...
    for idx, fq in enumerate(fake_questions):
        ...
        # 1차 의도 검증
        print(f"[F{f_num} - 1차 방어막] 질문 의도가 법률적 질문인지 검사 중...")
        is_legal = guardrail_agent.check_legal_intent(fq)
        if not is_legal:
            print(f"[F{f_num} - 결과] ✅ 1차 방어막(LLM) 즉시 차단 성공!")
            continue
        print(f"[F{f_num} - 결과] ⚠️ 1차 방어막 통과 (2/3차 검증 시작)")
            
        # 3차 스코어 컷오프
        print(f"\n[F{f_num} - 3차 방어막] Chroma 유사도 스코어 검색 및 컷오프 검사 중...")
        raw_results = vector_db.search_with_scores(fq, k=10)
        filtered_docs = guardrail_agent.filter_references(raw_results)
        
        # 2차 프롬프트 검증
        print(f"\n[F{f_num} - 4차 답변 생성] Gemma-4 시스템 프롬프트 환각 방지 검증 중...")
