...
class LegalParser:
    """
    Parser for AI Hub raw CSV data and metadata enrichment from JSON files.
    """
    ...
    def _pre_index_json_files(self):
        ...
        if not os.path.exists(self.labeled_data_path):
            return
        print("라벨링 데이터(JSON) 메타데이터 색인 중... (1회만 실행)")
    ...
    def parse_csv_file(self, file_path: str, doc_type: str) -> Dict[str, Any]:
        """
        Parse a single CSV file, combine sentences, and build metadata.
        """
        try:
    ...
    def load_all_documents(self) -> List[Dict[str, Any]]:
        ...
        for subdir in subdirs:
            ...
            print(f"Parsing {len(csv_files)} files in {subdir}...")
        ...
        print(f"Successfully parsed total of {len(documents)} documents.")
        return documents
