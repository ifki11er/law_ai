...
class GuardrailAgent:
    ...
    def analyze_query(self, query: str, history: list = None) -> dict:
        ...
        system_prompt = (
            ...
            "3. 'expanded_query' (string): 검색 정확도를 극대화하기 위해 맥락 속 모호한 대명사(예: '이거', '그거', '그 사람')를 명확한 지칭 대상으로 변환하고, 이전 대화 맥락에 등장한 중요 조건(예: '전치 5주', '음주 상태', '합의금 1억')이 현재 질문에 직접 기술되어 있지 않더라도 질문 해결에 결정적인 요소라면 검색어에 명시적으로 추가하여 확장한 검색어.\n"
