...
def main():
    print("\n" + "="*50)
    print("=== [인제스천 파이프라인 v3] 계층구조 & 하이브리드 DB 구축 시작 ===")
    print("="*50)
    
    # 0. 기존 DB 및 BM25 파일 초기화
    if os.path.exists(CHROMA_DB_PATH):
        print(f"\n[초기화] 기존 v3 DB 폴더({CHROMA_DB_PATH})를 초기화(삭제)합니다...")
        try:
            shutil.rmtree(CHROMA_DB_PATH)
            print("[초기화 완료] 기존 v3 DB 삭제 완료.")
        except Exception as e:
            print(f"⚠️ 경고: 기존 v3 DB 폴더 삭제 실패: {e}")
    ...
    print("\n[Step 1] AI Hub CSV 원천 데이터 로드 및 계층 구조 분할 시작...")
    ...
    # 2. 컨텍스트 합성형 부모-자식 분할
    print("\n[Step 2] 브레드크럼 접두사 합성 및 자식 청크 분할(v3) 시작...")
    ...
    print(f"\n[Step 4] Chroma DB v3 및 BM25 하이브리드 적재 시작...")
    ...
    print("\n" + "="*50)
    print("=== [인제스천 파이프라인 v3] 계층구조 & 하이브리드 RAG DB 구축 성공 완료! ===")
    print("="*50 + "\n")
