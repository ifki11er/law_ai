...
class LegalUpdateManager:
    ...
    def insert_article(self, 
        ...
        try:
            # We need to load/merge with existing BM25 data if it already exists
            # Wait, the add_documents in LegalVectorDB automatically appends or builds.
            # In our current LegalVectorDB, add_documents builds the index from the input chunks list.
            # Let's handle appending to BM25 correctly.
            if self.db.bm25_data is not None:
                existing_chunks = self.db.bm25_data["chunks"]
                # Append new chunks to existing chunks
                all_chunks = existing_chunks + chunks
            else:
                all_chunks = chunks
                
            self.db.add_documents(chunks) # Add to Chroma
            
            # Rebuild and save BM25 with all chunks (including new one)
            from rank_bm25 import BM25Okapi
            corpus_texts = [c["text"] for c in all_chunks]
            tokenized_corpus = [self.db.tokenize_korean(t) for c in all_chunks for t in [c["text"]]] # wait, tokenized_corpus
            tokenized_corpus = [self.db.tokenize_korean(t) for t in corpus_texts]
            ...
            with open(self.db.bm25_path, 'wb') as f:
                pickle.dump(bm25_payload, f)
            self.db.bm25_data = bm25_payload
