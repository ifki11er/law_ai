...
def run_evaluation():
    print("="*60)
    print("=== [로컬 RAG v3 자동 벤치마크 테스트기 가동] ===")
    print("="*60)
    ...
    # 2. RAG v3 컴포넌트 로드
    print("[2단계] 로컬 RAG v3 시스템 엔진(chroma_db_v3) 및 채점관(Evaluator) 로드 중...")
    embedding = LegalEmbedding()
    
    # Define v3 database path
    ...
    for idx, sc in enumerate(scenarios):
        ...
        # [검색 최적화: 질문 재작성 및 확장]
        expanded_query = llm_agent.rewrite_query(sc['question'])
        
        # Chroma v3 하이브리드 검색 (Top 10)
        print(f"\n[Q{q_num} - 2차 검색] BM25와 Chroma를 이용한 v3 하이브리드 검색 중...")
    ...
    # 가짜 질문 방어력 추가 테스트
    print("\n" + "="*60)
    print("=== [4단계] 가드레일 우회 및 오작동 방어력 테스트 (v3) ===")
    print("="*60)
